...
BOOST_AUTO_TEST_CASE( testDirectVsChainComplexityLadder )
{
    ...
    // two methods) must be < 1e-18 (enforced globally below). Observed values (in the trailing
    // comments) are 4-38 ps amplitude and ~1e-19--1e-20 slope, comfortably inside these bounds.
    const std::vector< LadderScenario > scenarios = {
        { "E1_Earth_Sun_PointMass", "Earth", { "Sun" }, { }, earthRadius, 30.0, 120.0, 1.0e-10 },  // observed ~38 ps
        { "E2_Earth_Sun_EarthSH20", "Earth", { "Sun" }, { { "Earth", { 20, 20 } } }, earthRadius, 30.0, 120.0, 1.0e-10 },  // observed ~38
                                                                                                                           // ps
        { "E3_Earth_SunMoon_EarthSH20_MoonSH2",
          ...
          30.0,
          120.0,
          1.0e-10 },                                                                                          // observed ~7.1 ps
        { "M1_Moon_EarthSun_PointMass", "Moon", { "Earth", "Sun" }, { }, moonRadius, 30.0, 120.0, 1.0e-10 },  // observed ~3.6 ps
    ...
    for( const LadderScenario& scenario : scenarios )
    {
        ...
        // ~2e-19 (Earth) / ~1e-20 (Moon). Guard at 1.5e-18 (~3x): a regression to a first-order chain
